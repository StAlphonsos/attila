...
;   ("MbongoMUCK"	TinyMUCK"mbongo.ucsd.edu"		4201)
    ...
    ("JaysHouseMOO"     MOO     "jhm.ccs.neu.edu"               1709)
    ("PARC"		Jupiter	"osprey.parc.xerox.com"		7777)

    ("jiveMOO"		MOO	"localhost"			7777)
    ))


(defvar mud-default-default-server "jiveMOO"
...
(if (not (boundp 'send-region))
    (defun send-region (proc start end &optional buffer)
      (process-send-region proc start end buffer)))

(if (not (boundp 'send-string))
    (defun send-string (proc string &optional start end)
      (process-send-string proc string start end)))
...
(autoload 'moo-code-mode "moo-code")
