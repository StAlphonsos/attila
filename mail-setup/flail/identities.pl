...
#
# Time-stamp: <2010-08-10 09:13:11 attila@stalphonsos.com>
...
sub become {
  ...
  unless (exists($IDENTITIES{$id})) {
    foreach my $k (keys %IDENTITIES) {
      if (addresses_match($IDENTITIES{$k},$id) || (lc($k) eq lc($id)) ||
          ($IDENTITIES{$k} =~ /$id/i)) {
        $id = $k;
        last;
      }
  ...
  my $previous_identity = $CurrentIdentity;
  $CurrentIdentity = $id;
  ...
  if ($coderef) {
    &$coderef();
    $id = become($previous_identity);
  }
  return $previous_identity;
}

sub become_filter {
    return become() unless @_;
    my($id,@rest) = @_;
    if ($id =~ /internal/i) {
        warn("[Not switching to internal identity: $id]\n");
        $id = 'Lytix';
    }
    become($id,@rest);
}
...
$AssumeIdentityCallback = \&become_filter;
